//! Lexer for the Jade programming language.
//!
...
pub enum TokenType {
    ...
    Pool,    // pool (bump/arena allocator)
    Tight,   // tight { } stack-only scope
    Recycle, // recycle (object recycling)
    Inline,  // @inline / inline (force inlining)
    Hot,     // @hot (optimization hint)
    Cold,    // @cold (optimization hint)
    Noescape,// noescape (pointer cannot escape)
    Smallvec,// smallvec<N> type
    BorrowSplit, // borrow_split (safe split borrow)
    Zeroize, // zeroize (secure wipe on drop)
...
impl Lexer {
    ...
    fn scan_identifier_or_keyword(&mut self) -> Result<Option<TokenType>, String> {
        ...
        let token_type = match text.as_str() {
            ...
            "smallvec" => TokenType::Smallvec,
            "borrow_split" => TokenType::BorrowSplit,
            "zeroize" => TokenType::Zeroize,
